...
/***********************************************************************
 * & improved responses for GroupItem WebSocket requests:
 * 3)	Enhanced WebSocket connection by adding  
 * 
 * @author Ervin Sirk
 * @since 1.8.0
 ************************************************************************/

@Path(GroupResource.PATH_GROUPS)
public class GroupResource {
    ...
    public static ItemBean createItemBean(Item item, boolean drillDown, String uriPath) {
    	...
    	bean.label = getLabel(item.getName());
    	bean.icon = getIcon(item.getName());    	
    	//bean.type = item.getClass().getSimpleName();
    	//bean.link = UriBuilder.fromUri(uriPath).path(ContextResource.PATH_CONTEXT).path(bean.name).build().toASCIIString();
    ...
    static public String getLabel(String itemname) {
        ItemUIRegistry registry = RESTApplication.getItemUIRegistry();
        if(registry!=null) {
 			String label = registry.getLabel(itemname);
			return label;
		}
        return null;
    }
    
    static public String getIcon(String itemname) {
        ItemUIRegistry registry = RESTApplication.getItemUIRegistry();
        if(registry!=null) {
 			String icon = registry.getIcon(itemname);
			return icon;
		}
        return null;
    }     
